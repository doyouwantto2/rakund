...
export default function InstrumentSelect(props: InstrumentSelectProps) {
  ...
  const displayName = () => {
    ...
    if (af && props.isLoading()) {
      return (
        props.availableInstruments().find((i) => i.folder === af)?.name ?? af
      );
    }
  ...
  const displayFormat = () => {
    ...
    if (af && props.isLoading()) {
      return (
        props.availableInstruments().find((i) => i.folder === af)?.format ?? ""
      );
    }
  ...
  return (
    <div ref={wrapperRef} class="relative shrink-0">
      ...
      <Show when={showList()}>
        ...
        >
          <div class="p-1">
            <For each={props.availableInstruments()}>
              {(inst) => {
                    ...
                    >
                      <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <Show when={isLoaded()}>
                            <span class="text-xs text-emerald-400 font-medium shrink-0">
                              LOADED
                            </span>
                          </Show>
                        </div>

                        <span class="text-zinc-500 group-hover:text-zinc-300 transition-colors shrink-0">
                          →
                        </span>
                      </div>
                    </button>
