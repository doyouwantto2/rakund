...
export default function LoadingIndicator(props: LoadingIndicatorProps) {
  const { isLoading, layer, progress } = props;
  ...
  return (
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black/90 backdrop-blur-md text-white px-6 py-4 rounded-2xl shadow-2xl border border-zinc-700/50 z-50">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <div class="flex items-center gap-2 text-xs text-zinc-400">
            <span class="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
      ...
      <div class="mt-3 w-full bg-zinc-800/50 rounded-full h-1 overflow-hidden">
        <div
          class="h-full bg-gradient-to-r from-emerald-500 to-purple-500 rounded-full transition-all duration-300 ease-out"
          style={{ width: progress ? `${progress.progress}%` : "60%" }}
        />
      </div>
